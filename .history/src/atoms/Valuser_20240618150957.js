...
function Valuser() {
    ...
    const unsubscribe = onSnapshot(checksRef, (snapshot) => {
      snapshot.docChanges().forEach((change) => {
        if (change.type === 'added') {
          const newCheck = { ...change.doc.data(), uid: change.doc.id }; // Include the document ID
          console.log('New check-in data: ', newCheck);
          if (!scannedData.find((item) => item.id === newCheck.uid)) {
  ...
  const handleSave = async (updatedUser) => {
    if (updatedUser) {
      ...
      console.log("Real ID :", updatedUser.id)
      try {
        await updateDoc(docRef, updatedUser); // Update the document in Firestore
        setOpenModal(false);
        setEditedData(null);
      } catch (error) {
    ...
    <div>
      ...
      <div className='card-data'>
        {scannedData.length > 0 ? (
          scannedData.map((data, data.id) => (
            <div key={uid} className='card-holder' onClick={() => handleEdit(data)}>
              ...
              <div className='card-right-side'>
                ...
                <p><strong>Document UID:</strong> {data.uid}</p> */}
