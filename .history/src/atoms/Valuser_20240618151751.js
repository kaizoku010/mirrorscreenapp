...
function Valuser() {
    ...
    const unsubscribe = onSnapshot(checksRef, (snapshot) => {
      snapshot.docChanges().forEach((change) => {
        if (change.type === 'added') {
          const newCheck = { ...change.doc.data(), id: change.doc.id }; // Include the document ID
          console.log('New check-in data: ', newCheck);
          if (!scannedData.find((item) => item.id === newCheck.id)) {
  ...
  const handleSave = async (updatedUser) => {
    if (updatedUser) {
      const docRef = doc(db, 'checks', updatedUser.id); // Ensure you use 'id'
      console.log("Document ID: ", updatedUser.id)
      try {
